...
impl Builder {
    ...
    fn add_item_raw(&mut self, type_id: u16, id: u16, size: usize)
        -> Result<&mut [i32], BuilderError>
    {
        let offset = match self.snap.offsets.entry(key(type_id, id)) {
            ...
            hash_map::Entry::Vacant(v) => {
                Snap::prepare_item_vacant(v, &mut self.snap.buf, size)?
            }
        }.clone();
        Ok(&mut self.snap.buf[to_usize(offset)])
    }
    pub fn add_item(&mut self, type_id: u16, id: u16, data: &[i32]) -> Result<(), BuilderError> {
        self.add_item_raw(type_id, id, data.len())?.copy_from_slice(data);
        Ok(())
    }
    fn add_packed<W>(&mut self, w: &mut W, type_id: u16, id: u16, size: usize, p: &mut Unpacker)
        -> Result<(), Error>
        where W: Warn<Warning>
    {
