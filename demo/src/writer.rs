...
impl Writer {
    pub fn new<W: io::Write + io::Seek + 'static>(
        ...
        TimelineMarkers { amount: 0, markers: [0; 64] }.write(&mut writer.file)?;
    ...
    fn write_chunk_impl(&mut self, kind: DataKind, data: Option<&[u8]>) -> Result<(), WriteError> {
        ...
        self.huffman.clear();
        HUFFMAN.compress(data, &mut self.huffman).expect("too long compression");
        ChunkHeader::Data {
            kind,
            size: self.huffman.len().assert_u16(),
        }.write(&mut self.file, WRITER_VERSION)?;
        self.file.write_all(&self.huffman).map_err(binrw::Error::Io)?;
        Ok(())
    }
