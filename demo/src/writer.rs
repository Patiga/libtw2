...
impl Writer {
    pub fn new<W: io::Write + io::Seek>(
        ...
        file.write_packed(&TimelineMarkers { timeline_markers: ArrayVec::new() }.pack())?;
    ...
    fn write_header<W: SeekableWrite>(&mut self, file: &mut W, ddnet: bool) -> Result<(), io::Error> {
    ...
    pub fn write_chunk<W: io::Write + io::Seek>(&mut self, file: &mut W, chunk: Chunk)
        -> Result<(), io::Error>
    {
        match chunk {
    ...
    pub fn write_tick<W: io::Write + io::Seek>(&mut self, file: &mut W, keyframe: bool, tick: Tick)
        -> Result<(), io::Error>
    {
    ...
    ) -> Result<(), io::Error>
    {
        ...
        ChunkHeader::Chunk(type_, buffer.len().assert_u32())
            .write(file, WRITER_VERSION)?;
        file.write(buffer)?;
        Ok(())
    }
    pub fn write_snapshot<W: io::Write + io::Seek>(&mut self, file: &mut W, snapshot: &[u8])
        -> Result<(), io::Error>
    {
        Self::write_chunk_impl(file, &mut self.buffer1, ChunkType::Snapshot, snapshot)
    }
    pub fn write_snapshot_delta<W: io::Write + io::Seek>(&mut self, file: &mut W, delta: &[u8])
        -> Result<(), io::Error>
    {
        Self::write_chunk_impl(file, &mut self.buffer1, ChunkType::SnapshotDelta, delta)
    }
    pub fn write_message<W: io::Write + io::Seek>(&mut self, file: &mut W, msg: &[u8])
        -> Result<(), io::Error>
    {
        self.buffer2.clear();
