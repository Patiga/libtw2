...
impl Reader {
              ...
              R: io::Read,
    {
        let header_version: format::HeaderVersionPacked =
            data.read_packed().on_eof(format::Error::TooShortHeaderVersion)?;
        ...
        let timeline_markers: format::TimelineMarkersPacked =
            data.read_packed().on_eof(format::Error::TooShortTimelineMarkers)?;
        let timeline_markers = timeline_markers.unpack(warn)?;
        let mut map_sha256: Option<[u8; 32]> = None;
        if version == format::Version::V6Ddnet {
            let map_sha256_uuid: [u8; 16] = data.read_packed().on_eof(format::Error::TooShortMapSha256)?;
            if map_sha256_uuid != SHA_256_EXTENSION {
                return Err(format::Error::NotMapSha256Extension.into());
            }
            map_sha256 = Some(data.read_packed().on_eof(format::Error::TooShortMapSha256)?);
        }
              ...
              R: io::Read,
    {
        ...
        match chunk_header {
            ...
            ChunkHeader::Chunk(type_, size) => {
                ...
                let huff_data = HUFFMAN.decompress(raw_data, self.buffer2.as_mut_slice())
