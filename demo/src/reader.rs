...
use crate::format::{MAX_SNAPSHOT_SIZE, TickMarker};
...
impl Reader {
        ...
        where W: Warn<Warning>,
    {
        ...
        match chunk_header { ChunkHeader::Tick { marker: TickMarker::Absolute(t), keyframe } => {
            ...
            ChunkHeader::Tick { marker: TickMarker::Delta(d), keyframe } => {
                match self.current_tick {
                    None => Err(ReadError::StartingDeltaSnapshot),
                    Some(t) => {
                        match t.checked_add(d.i32()) {
