...
impl Header {
    pub fn check<W: Warn<Warning>>(&self, warn: &mut W) {
        self.net_version.check_padding_warn(warn, Warning::WeirdNetVersion);
        self.map_name.check_padding_warn(warn, Warning::WeirdMapName);
        self.timestamp.check_padding_warn(warn, Warning::WeirdTimestamp);
    }
}
...
impl TimelineMarkers {
    ...
    pub(crate) fn check<W: Warn<Warning>>(&self, warn: &mut W) {
        if self.markers[self.amount.assert_usize()..64].iter().any(|n| *n != 0) {
...
impl ChunkHeader {
    pub(crate) fn read<R, W>(data: &mut R, version: Version, warn: &mut W)
        -> binrw::BinResult<Option<ChunkHeader>>
        where W: Warn<Warning>, R: io::Read + io::Seek
    {
        ...
        where W: io::Write + io::Seek
    {
        ...
        match *self {
            ChunkHeader::Tick { marker: TickMarker::Delta(dt), keyframe } => {
                assert!(dt <= version.max_tick_delta());
                assert!(!keyframe);
                let flags: u8 = CHUNKTYPEFLAG_TICKMARKER | CHUNKTICKFLAG_INLINETICK | dt;
                flags.write(file)?;
            },
            ChunkHeader::Tick { marker: TickMarker::Absolute(t), keyframe } => {
...
impl TickMarker {
    pub fn new(tick: i32, prev_tick: Option<i32>, keyframe: bool, version: Version)
        -> TickMarker
    {
