...
impl DemoReader {
    ...
    pub fn next_chunk<W: Warn<Warning>>(&mut self, warn: &mut W)
        ...
        match self.raw.read_chunk(wrap(warn))? {
            ...
            Some(RawChunk::Snapshot(snap)) => {
                ...
                self.snap = self.snap_reader.read(wrap(warn), swap, &mut unpacker).unwrap();
            ...
            Some(RawChunk::SnapshotDelta(dt)) => {
                ...
                self.delta.read(wrap(warn), obj_size, &mut unpacker)
                    .map_err(ReadError::Snap)?;
                self.old_snap.read_with_delta(wrap(warn), &self.snap, &self.delta)
                    .map_err(ReadError::Snap)?;
        ...
        where W: Warn<Warning>
    {
        ...
        for item in snap.items().filter(|item| item.type_id != 0) {
            let type_id = if item.type_id < u16::MAX / 4 {
                ...
                let uuid = self.uuid_index.get(&item.type_id)
